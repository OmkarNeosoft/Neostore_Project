...
router.post("/login", (req, res) => {
  ...
  registermodel.findOne({ email: email, password: password }, (err, data) => {
    if (err) {
      res.json({ err: 1, msg: "Invalid Username or Password" });
    } else if (data == null) {
      res.json({ err: 1, msg: "Invalid Username or Password" });
    } else {
      let payload = {
    ...
    if (err) {
      ...
      ins.save((e) => {
        if (e) {
          ...
          res.json({ err: 0, msg: "Congrats! successfully Registered!" });
        }
      });
    } else {
...
router.post("/forgotmodule", async (req, res) => {
 ...
 span {
  ...
  <h3>please use the following One Time Password (OTP):
...
router.get("/fetchproduct", (req, res) => {
  productmodel
    .find()
    .populate(["category_id", "color_id"]) //to reference documents in other collections.
    .then((product) => {
      console.log(product);
...
router.get("/productdetails/:id", (req, res) => {
  let id = req.params.id; // searches the URL path, body, and query string of the request (in that order) for the specified parameter
...
router.put("/updateprofile/:id", (req, res) => {
  ...
  registermodel.updateOne(
    ...
    (err) => {
      ...
      res.json({ msg: "Profile has been updated Succesfully" });
    }
  );
});
...
router.post("/addaddress", (req, res) => {
  registermodel.find({ email: req.body.email }, (err, data) => {
    if (err) {
      res.json({ err: 1, msg: "Error in adding address" });
    } else {
      let email = req.body.email;
...
router.post("/UserSelectedAddress", (req, res) => {
  let email = req.body.email;

  ordersmodel.updateOne(
    { email: email, Orderno: req.body.orderno },
    { $set: { selectaddr: req.body.selectaddr } },
    (err) => {
      if (err) res.json({ err: err });
